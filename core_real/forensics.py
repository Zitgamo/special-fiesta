...
class IronForensics:
    ...
    def log_trade(self, unit_id, symbol, side, volume, price, sl, tp, pnl=0, status='LIVE', ticket=None, comment=None):
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            if ticket:
                ...
                ''', (ticket, unit_id, symbol, side, volume, price, sl, tp, pnl, status, comment))
            else:
                # No ticket yet (PENDING)
                cursor.execute('''
                    INSERT INTO trades (unit_id, symbol, side, volume, price, sl, tp, pnl, type, comment)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (unit_id, symbol, side, volume, price, sl, tp, pnl, status, comment))
