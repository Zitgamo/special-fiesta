...
class SouthernPaperBridge:
    ...
    def log_trade(self, unit_id, trade):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO trades (unit_id, symbol, side, volume, price, sl, tp, type, pnl)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                unit_id, SYMBOL, trade['side'], 1.0, 
                trade['entry_p'], trade['sl'], trade['tp'], "PAPER", trade['pnl_pts']
            ))
        ...
        for uid, u in self.units.items():
            ...
            if u['pos'] == 0:
                ...
                if side != 0:
                    ...
                    u['entry'] = price
                    u['entry_t'] = now_t
                    u['sl'] = price - (atr * u['atr_sl'] * side)
                    u['tp'] = price + (atr * u['atr_tp'] * side)
                    self.logger.info(f"🐉 [{uid}] ENTRY {'LONG' if side==1 else 'SHORT'} @ {price} | SL: {u['sl']:.1f} TP: {u['tp']:.1f}")
