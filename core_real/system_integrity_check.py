...
def check_integrity():
    print("--- SOVEREIGN REAL SYSTEM INTEGRITY AUDIT ---")
...
def run_audit():
    ...
    results = {}
    base = get_base_path()
    core = os.path.join(base, "core_real")
    
    # 1. PILLAR: PERSISTENCE
    sentinel_alive = any("core_real/sentinel.py" in " ".join(p.cmdline()).lower() for p in psutil.process_iter(['cmdline']) if p.info['cmdline'])
