...
try:
    ...
    DB_PATH, DNA_PATH, CORE_DIR = "core_real/iron_core.db", "core_real/dna.json", "core_real"
...
class SovereignMaster:
    def __init__(self):
        self.units = {
            "ALPHA": "core_real/engine.py",
            "OMEGA": "core_real/engine.py",
            "GAMMA": "core_real/engine.py"
        }
    ...
    def perform_market_scan(self):
        ...
        # 2. Smart Watchlist Discovery (Dynamic Alpha)
        print(" >> [SMART_SCAN] Identifying High-Alpha Assets...")
        potential_symbols = ["US30", "DE30", "XAUUSD", "EURUSD", "GBPUSD", "XAGEUR", "XAGAUD", "XAGUSD", "NAS100", "US500"]
        
        # Add any symbol that MT5 has marked as "visible" or "favorite"
        ...
        if all_symbols:
            for s in all_symbols:
                ...
                if any(c in s.name.upper() for c in mt5_crypto_blacklist):
                    continue
                if s.visible and s.name not in potential_symbols:
                    potential_symbols.append(s.name)
        ...
        squadron = {"ALPHA": [], "OMEGA": [], "GAMMA": []}
        
        # 5. Elite Assignment
        # ALPHA: High Efficiency Crypto + Stable Forex
        squadron["ALPHA"] = pick_unique(crypto_results, 1) + pick_unique(results, 1)
        # OMEGA: High Volatility Trend-Catchers
        squadron["OMEGA"] = pick_unique(results, 2)
        # GAMMA: Secondary Diversification
        squadron["GAMMA"] = pick_unique(results, 2)

        # FALLBACK: If results are poor, inject Iron Classics to ensure 100% Squadron Population
        if sum(len(v) for v in squadron.values()) < 3:
            print(" !! [FAIL_SAFE] Scanner returned low signal. Injecting Iron Classics...")
            squadron["ALPHA"] = list(set(squadron["ALPHA"] + ["BTC/USDT", "ETH/USDT"]))[:2]
            squadron["OMEGA"] = list(set(squadron["OMEGA"] + ["XAUUSD"]))[:2]
            squadron["GAMMA"] = list(set(squadron["GAMMA"] + ["GBPUSD", "XAGUSD"]))[:2]

        # 6. Save
        with open("core_real/squadron.json", "w") as f:
            json.dump(squadron, f, indent=4)
        print(f" >> [SUCCESS] Elite Squadrons Assigned: {squadron}")
    ...
    def autonomous_asset_migration(self):
        ...
        squad_path = "core_real/squadron.json"
        try:
        ...
        try:
            ...
            while self.is_running:
                try:
                    ...
                    trigger_path = "core_real/scan_trigger.tmp"
